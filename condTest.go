...
func (c *copyChecker) check() {
	/**
	因为 copyChecker 的底层类型为 uintptr
	那么 这里的 *c其实就是 copyChecker 类型本身，然后强转成uintptr
	和拿着 c 也就是copyChecker 的指针去求 uintptr，理论上要想等
	即：内存地址为一样，则表示没有被复制
	 */
	 // 下述做法是：
	 // 其实 copyChecker 中存储的对象地址就是 copyChecker 对象自身的地址
	 // 先把 copyChecker 处存储的对象地址和自己通过 unsafe.Pointer求出来的对象地址作比较，
	 // 如果发现不相等，那么就尝试的替换，由于使用的 old是0，
	 // 则表示c还没有开辟内存空间，也就是说，只有是首次开辟地址才会替换成功
	 // 如果替换不成功，则表示 copyChecker 处所存储的地址和 unsafe计算出来的不一致
	 // 则表示对象是被复制了
