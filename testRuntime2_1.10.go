...
使用了 信号灯 的实现
 */
// 互斥锁。 在非竞争情况下，与自旋锁（只是一些用户级指令）一样快，
// 但在争用路径上它们在内核中休眠。一个归零的互斥锁被解锁（无需初始化每个锁）。
type mutex struct {

	// 基于Futex的impl将其视为uint32键，
	// 而基于sema的impl则视为M * 的waitm
	// ( waitm : 简短说就是等待一个命令执行过程n分钟)。
	// 两者的结合，但这种结合打破了精确的GC机制。
	// 【注】是一种用户态和内核态混合的同步机制。
	key uintptr
}
