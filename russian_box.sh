...
#位置与大小
marginLeft=3			#边框左边距 (3个光标的宽度)
marginTop=2			    #边框上边距 (两行的宽度)
#
#
# ((exp))结构扩展并计算一个算术表达式的
#
((mapLeft=marginLeft+2))	#棋盘左边距 (棋盘的左边边框左边)
((mapTop=$marginTop+1))		#棋盘上边距 (棋盘的上内边框坐标)
mapWidth=10			        #棋盘宽度 (严格来说只是内宽度的一半)
mapHeight=15			    #棋盘高度 (这个就是内高度)
...
iSumType=7			#方块类型总数： (正方形/长棍/Z形/反Z/7形/反7/卜形)
boxStyle=(1 2 2 2 4 4 4)	#各种方块旋转后可能的样式数目 (各种样式数目和类型逐一匹配， 如： `正方形`只有一种样式， `长棍` 有两种， `Z` 有两种 等等)
...
map=()			#棋盘图表 (记录当前棋盘中的状况, 已有的方块)

# 括号内支持多个表达式用逗号分开。只要括号中的表达式符合C语言运算规则
# 初始化所有背景方块为-1, 表示没有方块
# 即：初始化棋盘，用坐标 -1在对应位置上表示没有内容
# 后续有方块放置某些位置后，某些位置上对应的 -1 会被修改成其他数字， 以此来判断某些位置上是否可以继续放置内容
...
{
    # local 用于shell内局部变量的定义，多使用在函数内部
...
CreateBox()
{
    ...
    # 如果 boxCur 数组的长度为0
	if (( ${#boxCur[@]} == 0 )); then  # 刚开始的第一个方块？
		# 当前方块不存在
		# 通过 RANDOM 变量获取 (0--32767)
		(( iBoxCurType = RANDOM % iSumType ))                   # 随机出一种 方块类型
		(( iBoxCurStyle = RANDOM % ${boxStyle[$iBoxCurType]} )) # 随机出一种 方块旋转后的 样式
		(( iBoxCurColor = RANDOM % $iSumColor + 1 ))            # 随机一种颜色
	else
    ...
    # 方块出现
	if ! BoxMove $boxCurY $boxCurX # 如果刚一出现就不能移动(没有足够的移动空间)
	then
		kill -$sigExit $PPID # 结束进程，退出  kill -30 PID
		ShowExit
	fi
...
DrawBorder()
{
	...
	# mapWidth*2 才是真正的游戏框的宽度
	((x2 = x1 + 2 + mapWidth * 2))			# 右边框x坐标

	# 画出左右两条 竖线
	for ((i = 0; i < mapHeight; i++))
	do
		((y = i + marginTop + 2))           # 从第4行开始画竖线
    ...
    # 画出上下两条 横线
    # 将之前的x1 变量作为新 画横线的坐标
    # x1为第几行， 除去顶部和高度及两行的高度，作为下边框的起点
	((x1 = marginTop + mapHeight + 2))

	for ((i = 0; i < mapWidth + 2; i++))
	do
		((y = i * 2 + marginLeft + 1))      # 从第4列开始画横线
		echo -ne "\033[${mapTop};${y}H=="	# 绘制上边框
		echo -ne "\033[${x1};${y}H=="		# 绘制下边框
	done
	echo -ne "\033[0m"                      # 这个是打印一个空？

	#显示"Score"和"Level"字样
	echo -ne "\033[1m"


	((y = marginLeft + mapWidth * 2 + 7))    # 定义好 `Score` 和 `Level` 字样
	((x1 = marginTop + 10))
...
ShowExit()
{
	...
	echo -e "\033[${y};1HGameOver!\033[0m"  # 打印 `GameOver!`
	exit
}
